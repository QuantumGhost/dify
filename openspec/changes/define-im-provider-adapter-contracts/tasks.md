...
- [ ] 2.1 Add Provider-specific typed adapter configurations without flattening API credentials, applicable event material or typed Directory page-size seams into a generic map; keep DingTalk and WeCom free of event-transport fields.
...
- [ ] 5.1 Implement Basic Messaging `test_destination` and `send_text` for all five Providers using root-owned clients and exactly one explicit personal-user destination.
...
- [ ] 7.2 Perform each applicable API operation or event path against an authorized non-production Provider environment, allowing any Directory user to be selected for personal Messaging tests, and retain sanitized real request、response or event payload fixtures.
