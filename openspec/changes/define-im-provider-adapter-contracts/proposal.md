...
- 定义独立的 Provider Directory capability：一次读取在 concrete adapter 内完成 Provider-specific pagination 或 hierarchy traversal，并只在完整成功后返回 immutable snapshot。Slack、Feishu/Lark、DingTalk 与 Microsoft Teams 提供 typed positive `directory_page_size` test seam；WeCom 不提供伪分页配置。
- 定义必备的 personal-user-only Basic Messaging 与可选 Dynamic Card Messaging capabilities；所有 Provider 的新消息只允许面向一个 IM user，不暴露 group、channel、chat、department 或 tag broadcast destination。每次方法调用至多执行一次 side-effecting Provider operation，并返回 Provider acceptance 与精确 Provider-owned message reference。
...
- Capability presence 是支持情况的唯一事实来源：仅 Slack、Feishu/Lark 与 Microsoft Teams 暴露 Webhook Events；仅 Slack 与 Feishu/Lark 暴露 STREAM Events；DingTalk 与 WeCom 只暴露 credential、Directory 与 Basic Messaging，不实现 dummy capability。
...
- 影响 IM Provider adapter composition、credential/client lifecycle、directory readers、personal basic/card messaging adapters，以及 Slack、Feishu/Lark、Microsoft Teams 的 Webhook 和 Slack、Feishu/Lark 的 stream adapters。
...
- Messaging scope 不包含群聊、频道、部门、标签或其他广播发送；授权非生产测试企业可从 Directory 中选择任一用户作为 personal destination 候选，但不得由此恢复群发 destination surface。
