...
- [x] 3.3 实现 config resolver：CE deployment global；EE tenant override > deployment global；Cloud Slack ISV / DingTalk tenant self-built；deployment global 可来自 config/secret manager，不强制落 DB。
- [x] 3.4 实现 Slack ISV install/uninstall/token refresh 数据路径和接口边界；只有需要生命周期管理的 install/config 才落 DB。
...
- [x] 9.2 后续在 Web 侧新增 Contact 管理和 Contact recipient 配置入口，使用现有 i18n 规范添加文案。
- [x] 9.3 展示当前 account 的 IM binding status，并支持 revoke。
- [x] 9.4 增加 IM app config/install UI，包括 Cloud Slack ISV install 和 self-built credential 配置。
- [x] 9.5 对未绑定 IM 的 member Contact 展示 email fallback 状态，而不是静默失败。

## 10. Observability And Operations

- [x] 10.1 为 Contact seed、binding、delivery、callback、card compensation 和 resume enqueue 增加结构化日志。
- [x] 10.2 在 logs/status 中包含 tenant、app、workflow run/conversation、form、contact snapshot、provider、message 和 event identifiers。
- [x] 10.3 增加 provider config 校验，启动或首次调用时能发现缺失 credentials。
- [x] 10.4 定义 operator 排障路径，包含 Contact missing、delivery fallback/skip、callback rejected、form expired、card update failed 和 resume enqueue failed。
- [x] 10.5 确认 rollback 行为：关闭新 HITL/IM provider config 后不影响旧 HITL、Web/Email submission 和现有 workflow resume。
