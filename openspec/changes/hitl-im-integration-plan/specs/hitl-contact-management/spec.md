...
系统 SHALL 支持 Workspace-scoped Contact，其中 member Contact 引用 Dify Account，external Contact 只保存 name 和单个 email。
...
系统 SHALL 保证同一个 Workspace 内一个 Account 最多对应一个 active member Contact。
