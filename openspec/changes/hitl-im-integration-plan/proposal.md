...
- 新增 Workspace-scoped `Contact` 对象，用于表达 HITL 接收人；`Contact` 是 authoritative workspace recipient row，而不是运行时临时投影。member Contact 关联 Dify `Account`，其 profile 读路径以 `Account` 为准，而 `Contact.name/email` 仅作为 bootstrap/fallback cache；external Contact 的 profile 由 `Contact` 自身持有。demo 阶段提供从现有 Workspace members 创建 Contact 的脚本用于测试和演示数据准备。
...
- 新增 provider-neutral IM adapter 和 app config resolver，支持飞书企业自建 demo，并为 CE/EE 企业自建、Cloud Slack ISV、Cloud 钉钉企业自建预留配置来源边界。demo 中飞书企业自建应用使用 long connection 模式，而不是 webhook 模式。
...
- 数据兼容：不把现有 `TenantAccountJoin` 全量迁移到 Contact 表；demo 只提供从现有 Workspace members 创建 Contact 的脚本。成员 Contact 一旦创建，即作为 authoritative workspace recipient row 持久化；Contact 自动同步、lazy materialization 和正式迁移/投影策略后续单独设计。
