...
from tasks.mail_human_input_delivery_task import dispatch_human_input_email_task, dispatch_human_input_feishu_task
...
class WorkflowBasedAppRunner:
    ...
    def _enqueue_human_input_notifications(self, reasons: Sequence[object]) -> None:
        for reason in reasons:
            ...
            try:
                dispatch_human_input_feishu_task.apply_async(
                    kwargs={"form_id": reason.form_id, "node_title": reason.node_title},
                    queue="mail",
                )
            except Exception:  # pragma: no cover - defensive logging
                logger.exception("Failed to enqueue human input feishu task for form %s", reason.form_id)
