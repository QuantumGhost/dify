...
class FeishuIngressService:
    ...
    def handle_webhook_request(self, *, headers: Mapping[str, str], body: bytes) -> tuple[int, bytes]:
        ...
        if config is None:
            return 503, b"missing feishu provider config"
        if config.ingress_mode != HumanInputIMIngressMode.WEBHOOK:
            return 409, b"feishu ingress mode is not webhook"

        channel = self._get_or_create_channel(config)
        return asyncio.run(channel.handle_webhook_request(headers, body))

    def _get_or_create_channel(self, config: HumanInputIMProviderConfig):
        if self._channel is not None and self._channel_config == config:
            return self._channel

        channel = _build_feishu_channel(config, self._handle_card_action)
        channel.start()
...
def _build_feishu_channel(config: HumanInputIMProviderConfig, card_action_handler):
